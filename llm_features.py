...
def opt_batch_get_hidden_states(text: List[str], layer_idxs: List[int], llm, tokenizer):
...
def llama_batch_get_hidden_states(text: List[str], layer_idxs: List[int], llm, tokenizer):
    model_device = next(llm.model.parameters()).device

    token_data = tokenizer(text, return_tensors="pt", padding=True)
    token_ids = token_data["input_ids"].to(model_device)
    attention_mask = token_data["attention_mask"].to(model_device)
    hidden_states = llm.model(input_ids=token_ids, attention_mask=attention_mask, output_hidden_states=True).hidden_states
...
class OPTEmbeddingGenerator:
    ...
    def process_batched_text(self, text: List[str]) -> torch.Tensor:
        hidden_states, attention_mask = opt_batch_get_hidden_states(text, self.layer_idxs, self.llm, self.tokenizer)

        pool_with_mask = lambda hidden_state: self.pool_function({"token_embeddings": hidden_state, "attention_mask": attention_mask})["sentence_embedding"]
...
class LLaMaEmbeddingGenerator:
    def __init__(self, pooling_methods: List[str], layers: List[int], device="cuda:0") -> None:
        print("Loading tokenizer...")
        self.tokenizer = AutoTokenizer.from_pretrained("yahma/llama-7b-hf")
    ...
    def process_batched_text(self, text: List[str]) -> torch.Tensor:
        hidden_states, attention_mask = llama_batch_get_hidden_states(text, self.layer_idxs, self.llm, self.tokenizer)
...
class EmbeddedHHRLHF:
    def __init__(self, pooling_methods: List[str], layers: List[int], llm_embedding=OPTEmbeddingGenerator, batch_size: int = 16, data_dir: str = None, split: str="train", device="cuda:0") -> None:
        # super().__init__(pooling_methods, layers, device)
        self.llm_embedding_generator = llm_embedding(pooling_methods, layers, device)
        self.split = split
    ...
    def next(self) -> torch.Tensor:
        ...
        with torch.no_grad():
            embeddings = self.llm_embedding_generator.process_batched_text(all_text)
